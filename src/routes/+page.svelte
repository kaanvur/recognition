<script lang="ts">
	...
	import { Label, Fileupload, Heading, List, Li, Card, Spinner } from 'flowbite-svelte';
	...
	const classifyImage = async (model: mobilenet.MobileNet, preprocessedImage: tf.Tensor3D) => {
...
<svelte:head>
	...
	<meta name="description" content="A simple ML Vision app" />
</svelte:head>
...
<section class="flex flex-col gap-12">
	...
	<div class="ml-auto mr-auto block w-1/2">
		...
		<Fileupload {...fileuploadprops} bind:files on:change={handleImageChange} />
	</div>
	<Card size="md" padding="sm" class="ml-auto mr-auto">
		<img hidden={!imgDisplay} src={imgDisplay} alt="" bind:this={imgtoprocess} />
		<div class="py-4">
			<Heading tag="h3">Predictions:</Heading>
			<List position="outside" class="p-4">
				{#if predictionsLoading}
					<span class="flex flex-col items-center"><Spinner color="blue" /></span>
				{:else}
					{#each predictionsResult as prediction}
						<Li>{prediction.className}: {Math.round(prediction.probability * 100)}%</Li>
					{/each}
				{/if}
			</List>
		</div>
